...
void AdmCommonDefinitionHelper::populateElementRelationshipsFor(adm::TypeDescriptor typeDescriptor)
{
    ...
    tdData->id = typeDescriptor.get();//ID of type, e.g. 4 for HOA
    tdData->typeDescriptor = typeDescriptor; //actual typeDescriptor
    tdData->name = adm::formatTypeDefinition(typeDescriptor);//nice way of writing
